...
if uploaded_file is not None:
    ...
    filtered_data = df[df['Cluster'].isin(selected_clusters) &
    ...
    change_df = df.groupby(['Cluster', 'TP']).apply(calculate_percentage_change).reset_index(
        name='Change 1. - last [%]')

    # Extrahiere den Wert von "IfcElemente" des aktuellsten Datums jeder Gruppe
    ifc_elements_latest = df.groupby(['Cluster', 'TP']).apply(
        lambda x: x.loc[x['Datum'].idxmax(), 'IfcElemente']).reset_index(name='currentIfcElements')
